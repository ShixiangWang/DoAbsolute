...
DoAbsolute = function(Seg, Maf = NULL,
  ...
  #-- check Maf data
  if(!is.null(Maf)) {
    maf_cols = c("Tumor_Sample_Barcode", "Hugo_Symbol", "Chromosome", "Start_position", "dbSNP_Val_Status",
                 "t_ref_count", "t_alt_count")
    maf_cols2 = c("Tumor_Sample_Barcode", "Hugo_Symbol", "Chromosome", "Start_Position", "dbSNP_Val_Status",
                  "t_ref_count", "t_alt_count")
    if (all(maf_cols %in% colnames(Maf))) {
      Maf = Maf[, c("Tumor_Sample_Barcode", "Hugo_Symbol", "Chromosome", "Start_position", "dbSNP_Val_Status",
                    "t_ref_count", "t_alt_count")]
  ...
  files_seg = file.path(file.path(reviewed.dir, "SEG_MAF"),
                        grep("segtab.txt", dir(file.path(reviewed.dir, "SEG_MAF")), value = TRUE))
  t = file.copy(from = files_seg, to = seg.dir)
  files_maf = file.path(file.path(reviewed.dir, "SEG_MAF"),
  ...
  if (keepAllResult) {
    ...
    files_samplesCalled = file.path(file.path(reviewed.dir, "samples"),
